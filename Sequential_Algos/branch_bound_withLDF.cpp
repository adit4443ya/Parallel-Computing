...
set<int> branchAndBoundMIS(const Graph &graph, set<int> currentSet, int currentVertex) {
    ...
    // vector<pair<int, int>> verticesByDegree = graph.getVerticesByDegree();
    ...
    if (!graph.isAdjacentToSet(nextVertex, currentSet)) {
        ...
        set<int> withCurrentResult = branchAndBoundMIS(graph, withCurrent, currentVertex + 1);

        set<int> withoutCurrentResult = branchAndBoundMIS(graph, currentSet, currentVertex + 1);
